...
export class RequestHandler {
  ...
  private async handleMeasureWorkflowExecute(params: any): Promise<CommandResult> {
    ...
    try {
      ...
      // Download measures if requested
      if (params.downloadMeasures) {
        logger.info('Downloading measures for workflow');
        const downloadedMeasures = await measureApplicationWorkflow.downloadWorkflowMeasures(params.workflow);
        logger.info(`Downloaded ${downloadedMeasures.length} measures for workflow`);
      }
      
      // Validate the workflow if requested
      if (params.validateBeforeExecution) {
        logger.info('Validating workflow before execution');
        const validationResult = await measureApplicationWorkflow.validateWorkflow(params.workflow);
    ...
    try {
      ...
      return {
        ...
        output: validationResult.valid 
          ? `Workflow ${params.workflow.name} is valid` 
      ...
      logger.error({ params, error }, 'Error validating measure workflow');
