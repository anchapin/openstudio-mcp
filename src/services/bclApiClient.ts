...
export class BCLApiClient implements BCLIntegration {
  private readonly apiBaseUrl: string = 'https://bcl.nrel.gov/api/v1';
  ...
  async searchMeasures(query: string): Promise<Measure[]> {
    try {
      ...
      // Make the API request to search for measures
      const response = await this.httpClient.get(`/search/?fq[]=bundle:measure&fq[]=openstudio_version:*&q=${encodedQuery}`);
      ...
      const measures: Measure[] = response.data.data.map((item: any) => this.transformBclMeasureToMeasure(item));
      ...
      logger.error(`Error searching BCL measures: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  async downloadMeasure(measureId: string): Promise<boolean> {
    try {
      ...
      const downloadUrl = response.data.data.files.find((file: any) => file.fileName.endsWith('.zip'))?.downloadUrl;
      ...
      logger.error(`Error downloading BCL measure: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
  ...
  async installMeasure(measureId: string): Promise<boolean> {
    try {
      ...
      logger.error(`Error installing BCL measure: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
  ...
  async recommendMeasures(context: string, modelPath?: string): Promise<Measure[]> {
    try {
      ...
      const scoredMeasures = this.calculateMeasureScores(uniqueMeasures, context, modelPath !== undefined);
      ...
      logger.error(`Error recommending BCL measures: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  private async downloadTopRecommendedMeasures(measures: Measure[]): Promise<void> {
    try {
      ...
      logger.error(`Error downloading top recommended measures: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async downloadAndInstallMeasureIfNeeded(measureId: string): Promise<boolean> {
    try {
      ...
      logger.error(`Error downloading and installing measure ${measureId}: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
  ...
  async updateMeasure(measureId: string): Promise<boolean> {
    try {
      ...
      if (currentVersion === latestVersion) {
        logger.info(`Measure with ID ${measureId} is already up to date (version ${currentVersion})`);
        return true;
      }
      
      // Download and install the latest version
      logger.info(`Updating measure with ID ${measureId} from version ${currentVersion} to ${latestVersion}`);
      ...
      const installSuccess = await measureManager.installMeasureFromZip(zipFilePath, measureId, { force: true });
      ...
      logger.error(`Error updating BCL measure: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
   ...
   */
  private transformBclMeasureToMeasure(bclMeasure: any): Measure {
    try {
      // Extract measure arguments
      const args: MeasureArgument[] = bclMeasure.attributes?.arguments?.map((arg: any) => ({
        name: arg.name || '',
        displayName: arg.display_name || arg.name || '',
  ...
  private analyzeContext(context: string): { keywords: string[], categories: string[] } {
    ...
    const commonWords = new Set([
      'the', 'and', 'or', 'a', 'an', 'in', 'on', 'at', 'to', 'for', 'with', 'by', 'of', 
  ...
  private identifyCategories(context: string): string[] {
    ...
    const categoryPatterns: { [key: string]: RegExp[] } = {
      'hvac': [
        /hvac/i, /heating/i, /cooling/i, /ventilation/i, /air conditioning/i,
        /heat pump/i, /furnace/i, /boiler/i, /chiller/i, /fan/i, /duct/i
      ],
      'lighting': [
        /lighting/i, /light/i, /lamp/i, /luminaire/i, /fixture/i, /led/i,
        /daylight/i, /illumination/i, /glare/i
      ],
      'envelope': [
        /envelope/i, /insulation/i, /wall/i, /roof/i, /ceiling/i, /floor/i,
        /window/i, /glazing/i, /door/i, /infiltration/i, /air leakage/i
      ],
      'renewable': [
        /renewable/i, /solar/i, /photovoltaic/i, /pv/i, /wind/i, /geothermal/i,
        /biomass/i, /biofuel/i
      ],
      'water': [
        /water/i, /hot water/i, /domestic hot water/i, /dhw/i, /plumbing/i,
        /shower/i, /faucet/i, /toilet/i, /irrigation/i
      ],
      'controls': [
        /control/i, /thermostat/i, /sensor/i, /automation/i, /schedule/i,
        /setpoint/i, /setback/i, /occupancy/i, /demand response/i
      ],
      'plug_loads': [
        /plug load/i, /appliance/i, /equipment/i, /computer/i, /server/i,
        /refrigerator/i, /freezer/i, /dishwasher/i, /washer/i, /dryer/i
      ],
      'whole_building': [
        /whole building/i, /building performance/i, /energy model/i, /simulation/i,
        /calibration/i, /baseline/i, /benchmark/i, /audit/i, /retrofit/i
      ],
      'economics': [
        /cost/i, /saving/i, /economic/i, /financial/i, /payback/i, /roi/i,
        /investment/i, /utility/i, /bill/i, /tariff/i, /incentive/i, /rebate/i
      ]
    };
  ...
  private async getModelBasedRecommendations(modelPath: string): Promise<Measure[]> {
    try {
      ...
      logger.error(`Error getting model-based recommendations: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
   ...
   */
  private calculateMeasureScores(measures: Measure[], context: string, hasModelPath: boolean): { measure: Measure, score: number }[] {
  ...
  private calculateRelevanceScore(measure: Measure, context: string): number {
    ...
    const tagsLower = measure.tags.map(tag => tag.toLowerCase());
    if (context.split(' ').some(word => word.length > 2 && tagsLower.some(tag => tag.includes(word)))) {
      score += 1;
    }
  ...
  private handleApiError(error: AxiosError): void {
    if (error.response) {
      ...
      logger.error(`BCL API error: ${error.response.status} - ${JSON.stringify(error.response.data)}`);
