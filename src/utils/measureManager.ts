 ...
 */
export async function getMeasureVersion(measureId: string, customDir?: string): Promise<string | null> {
  try {
    const measurePath = getMeasurePath(measureId, customDir);
    ...
    const xmlContent = await fileOperations.readFile(measureXmlPath, { encoding: 'utf8' }) as string;
    ...
    logger.error(`Error getting measure version: ${error instanceof Error ? error.message : String(error)}`);
    return null;
  }
}
 ...
 */
export async function downloadMeasureFile(downloadUrl: string, options: MeasureDownloadOptions = {}): Promise<string> {
  ...
  try {
    ...
    logger.error(`Error downloading measure: ${error instanceof Error ? error.message : String(error)}`);
    throw new Error(`Failed to download measure: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function validateMeasureZip(zipFilePath: string): Promise<boolean> {
  try {
    ...
    const hasMeasureXml = entries.some(entry => 
      entry.entryName === 'measure.xml' || 
      entry.entryName.endsWith('/measure.xml')
    );
    ...
    const hasMeasureRb = entries.some(entry => 
      entry.entryName === 'measure.rb' || 
      entry.entryName.endsWith('/measure.rb')
    );
    ...
    logger.error(`Error validating measure zip file: ${error instanceof Error ? error.message : String(error)}`);
    return false;
  }
}
...
export async function extractMeasureZip(
  ...
  try {
    ...
    logger.error(`Error extracting measure zip file: ${error instanceof Error ? error.message : String(error)}`);
    throw new Error(`Failed to extract measure zip file: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function installMeasureFromZip(
  ...
  try {
    ...
    logger.error(`Error installing measure: ${error instanceof Error ? error.message : String(error)}`);
    return false;
  } finally {
    ...
    try {
      ...
      logger.warn(`Error deleting temporary zip file: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
export async function listInstalledMeasures(customDir?: string): Promise<string[]> {
  try {
    ...
    logger.error(`Error listing installed measures: ${error instanceof Error ? error.message : String(error)}`);
    return [];
  }
}
